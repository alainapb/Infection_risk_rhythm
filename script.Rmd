...
Hmm, maybe length at exposure isn't that informative?
...
```{r}
lrtest(mod_list[[2]], mod_list[[3]])
lrtest(mod_list[[3]], mod_list[[4]])
```

```{r}
# take closer look at model 7 which I think is reasonable based on EDA
summary(mod_list[[7]])
```
...
zi_nb_lin_age_both_int <- zeroinfl(spore_counts ~ Age * Time_treatment | Age + Time_treatment,
       dist = "negbin", link = "logit", data = reg_data)
zi_nb_lin_age_both_main <- zeroinfl(spore_counts ~ Age + Time_treatment | Age + Time_treatment,
       dist = "negbin", link = "logit", data = reg_data)
...
AIC(zi_nb_lin_age_int,
    zi_nb_lin_age_main,
    zi_nb_lin_age_both_int,
    zi_nb_lin_age_both_main) %>%
...
```{r}
zero_summary$response <- predict(zi_nb_lin_age_both_main, newdata = zero_summary, type = "response")
zero_summary$count <- predict(zi_nb_lin_age_both_main, newdata = zero_summary, type = "count")
zero_summary$prob_zero <- predict(zi_nb_lin_age_both_main, newdata = zero_summary, type = "zero")

zero_summary %>% knitr::kable(digits = 2)
```

```{r}
zero_summary$response <- predict(mod_list[[2]], newdata = zero_summary, type = "response")
zero_summary$count <- predict(mod_list[[2]], newdata = zero_summary, type = "count")
zero_summary$prob_zero <- predict(mod_list[[2]], newdata = zero_summary, type = "zero")
