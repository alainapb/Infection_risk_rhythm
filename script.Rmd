...
```{r}
AIC(inf_time, inf_main, inf_fr, inf_full) %>%
  mutate(deltaAIC = AIC - min(AIC)) %>% add_column(model = c("time", "Age + time", "Age + fr + time", "Age*time*fr"), .before = 1)
```
...
inf_preds <- 
  predict(inf_full, newdata = prev, type = "link", se.fit = TRUE) %>% 
  as_tibble() %>%
  mutate(half = crit * se.fit,
         lwr = fit - half,
         upr = fit + half,
...
CI_OR_night <- confint(inf_full, parm = "Time_treatmentNight", level = 0.95)
CI_OR_day <- confint(inf_full, parm = "(Intercept)", level = 0.95)
CI_OR_age <- confint(inf_full, parm = "Age", level = 0.95)
...
exp(coef(inf_full)[4])  #246% increase (-99% to )
exp(CI_OR_night)

# transform to probability scale
# point estimate and  95 % likelihood ratio interval
# day
boot::inv.logit(coef(inf_full)[1]) 
boot::inv.logit(CI_OR_day)
# night
boot::inv.logit(coef(inf_full)[4])
...
lrtest(zi_nb_null, zi_nb_all) #0.062
lrtest(zi_nb_time, zi_nb_all) # *0.052
lrtest(zi_nb_age_time, zi_nb_all) # * 0.0375
```
...
#Full model for infection intensity
``` {r}
library(emmeans)

predict(zi_nb_all, newdata = expand.grid(Time_treatment = c("Day", "Night"), Age = 6:9, fr_sw = seq(-0.5, 0.8, .2)), type = "response")
#predict(zi_nb_all, newdata = expand.grid(Time_treatment = c("Day", "Night"), Age = 6:9, fr_sw = seq(-0.5, 0.8, .2)), type = "count")
#predict(zi_nb_all, newdata = expand.grid(Time_treatment = c("Day", "Night"), Age = 6:9, fr_sw = seq(-0.5, 0.8, .2)), type = "zero")
...
nd_time2 <- zero_summary %>% dplyr::select(Age, Time_treatment, fr_sw)
emm_response_all2 <- emmeans(zi_nb_all, ~ Time_treatment | Age, at = list(Age = 6:9)) #what does ~ Time | Age do?
(emm_response_all2_df <- emm_response_all2 %>% data.frame())

emm_count_all_df <- emm_count_all %>% data.frame() %>% rename("count_time" = emmean, "count_lcl" = asymp.LCL, "count_ucl" = asymp.UCL)

nd_time2$count_time <- predict(zi_nb_all, newdata = nd_time2, type = "count") 
nd_time2$response_time <- predict(zi_nb_all, newdata = nd_time2, type = "response") #why are these different from above

#nd_time2 <- inner_join(nd_time2, emm_count_all_df, by = c("Time_treatment", "count_time")) 
#nd_time2 <- inner_join(nd_time2, emm_response_all_df, by = c("Time_treatment"))
