...
no_size<-po_correct %>%
  ggplot() +
  geom_point(aes(x = time, y = log(PO), group = sample)) +
  geom_smooth(aes(x = time, y = log(PO)), method = "lm", color ="orange", alpha = 0.5) +
...
with_size<-po_correct %>%
  ggplot() +
  geom_point(aes(x = time, y = log(size_cor), group = sample)) +
  geom_smooth(aes(x = time, y = log(size_cor)), method = "lm", color ="orange", alpha = .5) +
  labs(y="Active PO", x="Age (days)")+
  facet_grid(~Age, scales = "free_x")+
  scale_x_continuous(breaks = c(24,48,72,96), labels = c(6,7, 8,9))+
  theme_bw()+
...
summary(po_model<-glm(PO ~ Phase*as.factor(Age), data=po_correct))
plot(po_model)

summary(lm(log(PO) ~ Age, data = po_correct))
summary(lm(log(PO) ~ as.factor(Age)-1, data = po_correct))
...
newdata %>%
  ...
  geom_point(po_correct, mapping=aes(x=Age, y=PO, color=Phase), alpha = 0.5)
```
...
fit.log.po <- lm(log(PO) ~ Age + Time_treatment, data = po_correct)
summary(fit.log.po)
plot(fit.log.po)
...
ggplot() +
  ...
  geom_point(data = po_summary, mapping = aes(x = Age, y = age_mean_po, col = Time_treatment), pch = 22)
  #geom_jitter(data = po, mapping = aes(x = Age, y = Corrected_Change, col = Time_treatment), width = 0.1, height = 0)

summary(glm(PO ~ Age * Time_treatment, family = gaussian(link = "log"), data = po_correct))

```

```{r}
#model of PO by age 
summary(po_age<-glm(PO ~ Age, family = gaussian(link = "log"), data = po_correct))
with(summary(po_age), 1 - deviance/null.deviance) #Rsquared


#look at size corrected PO
summary(po_sc_age<-glm(size_cor ~ Age, family = gaussian(link = "log"), data = po_correct))
with(summary(po_sc_age), 1 - deviance/null.deviance) #rsquared
```
...
inf_fr <- glm(infected ~ Age + fr_sw + Time_treatment, family = binomial, data = reg_data)
...
#model6
inf_full <- glm(infected ~ Age * fr_sw * Time_treatment, family = binomial, data = reg_data)
...
```{r}
AIC(inf_time, inf_main, inf_fr, inf_full_lin, inf_full_cat) %>%
  mutate(deltaAIC = AIC - min(AIC)) %>% add_column(model = c("time", "Age + time", "Age + fr + time", "Age*time", "factor(Age)*time"), .before = 1)
```
...
```{r}
# get CI for plotting response
#model_mat <- model.matrix(zi_nb_all) %>% unique()
#rownames(model_mat) <- c("age6_day", "age6_night",
#                      "age7_day", "age7_night",
#                      "age8_day", "age8_night",
#                      "age9_day", "age9_night")
