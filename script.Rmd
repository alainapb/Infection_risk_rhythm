...
summary(spore_model<-glm(spore_counts ~ as.factor(Age) * Time_treatment, family = poisson(), data=reg_data))
plot(spore_model)
```
...
```{r}
zero_summary <- 
  all.data %>% filter(Exposed.Control == "Exposed") %>%
  group_by(Age, Time_treatment) %>%
...
Zero infections in Age 6 Day group. Can't look at interaction if fitting by MLE.

```{r}
all.data %>% filter(Exposed.Control == "Exposed") %>%
  group_by(Age, Time_treatment) %>%
  filter(Age == 6, Time_treatment == "Day", Exposed.Control == "Exposed") 
